...
def weight_variable(shape, args):
    initial = tf.random.truncated_normal(shape, stddev=args.std_dev)  # added
...
def run(args):
    ...
    cell_mut_dict = np.load(str(datadir) + "/" + "cell_mut_matrix.npy", encoding="latin1", allow_pickle=True).item()
    ...
    # define model
    drug = tf.compat.v1.placeholder(tf.float32, shape=[None, args.length_smiles, args.num_chars_smiles])  # added
    cell = tf.compat.v1.placeholder(tf.float32, shape=[None, args.num_cell_features])  # added
    ...
    fc1_w = weight_variable([shape[1] * shape[2], args.dense[0]], args)  # added
    fc1_b = bias_variable([args.dense[0]], args)  # added
    fc1_h = tf.nn.relu(tf.matmul(conv_flat, fc1_w) + fc1_b)
    fc1_drop = tf.nn.dropout(fc1_h, keep_prob)

    fc2_w = weight_variable([args.dense[1], args.dense[1]], args)  # added
    fc2_b = bias_variable([args.dense[1]], args)  # added
    fc2_h = tf.nn.relu(tf.matmul(fc1_drop, fc2_w) + fc2_b)
    fc2_drop = tf.nn.dropout(fc2_h, keep_prob)

    fc3_w = weight_variable([args.dense[2], 1], args)  # added
    fc3_b = weight_variable([1], args)
    ...
    # define optimizer
    train_step = tf.compat.v1.train.AdamOptimizer(args.learning_rate).minimize(loss)  # added learning rate
